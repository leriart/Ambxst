#!/usr/bin/env python3
"""
MonitorsWriter — nwg-displays compatible backend for Ambxst
===============================================================
...
NL   = os.path.expanduser("~/.local/share/ambxst")
...
def generate(monitors):
    ...
    hdr = f"# Generated by Ambxst on {ts}. Do not edit manually."
    conf = [hdr, ""]
    lua  = ["-- " + hdr[2:], ""]
    for m in monitors:
...
def inject_nl(conf_lines, lua_lines):
    for ext, lines, sm, em in [
        ("hyprland.conf", conf_lines, "# === AMBXST MONITORS ===", "# === END MONITORS ==="),
        ("hyprland.lua",  lua_lines,  "-- === AMBXST MONITORS ===", "-- === END MONITORS ==="),
    ]:
