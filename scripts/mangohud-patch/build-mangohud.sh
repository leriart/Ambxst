#!/usr/bin/env bash
# Build MangoHud modificado con output SHM para Ambxst
set -euo pipefail
...
echo "=== Building MangoHud $MANGOHUD_VERSION with Ambxst FPS output ==="
...
if ! grep -q "ambxst_fps" src/overlay.cpp 2>/dev/null; then
    ...
    if [ -n "$LINE" ]; then
        sed -i "${LINE}a\\
        // Write FPS to /dev/shm/ambxst_fps for Ambxst notch\\
        FILE *nfps = fopen(\"/dev/shm/ambxst_fps\", \"w\");\\
        if (nfps) {\
            fprintf(nfps, \"fps=%.1f\\npid=%d\\nframes=%lu\\nsource=mangohud\\n\",\\
...
cp build/src/mangohud "$INSTALL_DIR/../bin/mangohud-ambxst" 2>/dev/null || true
