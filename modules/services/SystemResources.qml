...
Singleton {
    ...
    // ── Config persistence ──
    property string metricsConfigPath: Quickshell.env("HOME") + "/.config/ambxst/config/metrics.json"
    ...
    // ── Fast FPS watcher (tail -f /dev/shm/ambxst_fps) ──
    property Process fpsWatcher: Process {
        ...
        command: ["bash", "-c", "tail -n0 -F /dev/shm/ambxst_fps 2>/dev/null || sleep 10"]
        stdout: SplitParser {
            onRead: data => {
